---
title: Development
slug: /development
---

<!--
 Copyright (c) 2024 Fraunhofer-Gesellschaft zur Förderung der angewandten Forschung e.V.

 This work is licensed under the Fraunhofer License (on the basis of the MIT license)
 that can be found in the LICENSE file.
-->

import PatentHint from './../patent-hint.mdx'

<PatentHint components={props.components} />

# Development Overview

## Target Group
If you are a **developer** and want to / are interested in ...

- ... deep insight about the Innamark project,
- ... adapt the library to your specific needs
- ... contributing to the project

the following pages provide you with the necessary information.

If you are a **developer** interested in just *using* the Innamark watermarking library inside your
own implementation without changing the core, please refer to the [Usage](/docs/usage/) section.

## Structure
They structure is based on the provided components.

```mdx-code-block
import DocCardList from '@theme/DocCardList';

<DocCardList />
```
